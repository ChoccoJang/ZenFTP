...
### 추가됨
- VSCode 재시작 후 파일 저장 시 자동 재연결 기능
- 워크스페이스 상태 저장/복원을 통한 FTP 파일 추적 유지
- 재연결 관련 다국어(i18n) 메시지 추가
- 다중 서버 환경에서 파일별 서버 정보 저장으로 올바른 서버로 재연결

### 수정됨
- FTP로 열린 파일만 저장되도록 검증 로직 추가
- 일반 파일 저장 시 오류 메시지 발생 문제 해결
- 파일 열기 시 발생하던 오타 수정 (tempFiileName → tempFileName)
- 여러 서버에서 연 파일을 각각 올바른 서버로 재연결하도록 개선

### 변경됨
- 저장 핸들러를 별도 메서드로 리팩토링 (setupSaveHandler)
- 코드 주석을 한글로 통일
- tempFileMap 구조를 확장하여 파일별 서버 정보 포함
